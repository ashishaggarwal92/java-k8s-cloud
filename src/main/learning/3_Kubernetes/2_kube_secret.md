...
kubectl create secret generic es-transport --from-file=tls.crt=transport/es-transport.crt --from-file=tls.key=transport/es-transport.key --from-file=ca.crt=ca/ca.cr
...
kubectl create secret generic es-http --from-file=tls.crt=http.crt --from-file=tls.key=http.key
